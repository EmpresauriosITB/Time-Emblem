...
public class DamageBehaviour : AbilityBehaviour {
    ...
    public override bool doAbility(int Power, bool isPhysical, List<GameObject> targets, GameObject actor) {
        ...
        for (int i = 0; i < targets.Count; i++) {
            ...
            if (targets[i].gameObject.GetComponent<CharacterUnitController>().isHumanoid) { targets[i].gameObject.GetComponent<CharacterUnitController>().animator.SetTrigger("LightHitTrigger"); }
